...
6. DeepSeek 分析输入改为服务端 D1 数据，并将输入指纹、模型版本、结果和后续真实重置结局保存为快照。
7. `/api/refresh` 提供带 Bearer Token 的定时器接入点，普通访客不能强制刷新。
...
- `analysis_snapshots`：保存可审计预测快照、前后变化和后续重置结局。
...
- **低流量采样空档**：页面访问仍会按新鲜度触发刷新；生产环境可以用受保护的 `/api/refresh` 接入外部定时器。由于当前 Site 为 `Only you`，外部定时器还需要解决 Sites 访问门禁，不能只配置应用层 Token。
- **外部 API 成本与密钥**：分析已由服务端控制并按证据指纹缓存，访客无法把任意文本转发给 DeepSeek。
- **公开测试版限制**：Sites 的可用性与存储额度受套餐、地区和工作区设置影响，发布前要在当前账号中确认。

## 后续可选优化

1. 在私有生产环境观察 FxTwitter 出站请求、D1 幂等写入与 7 天聚合。
2. Site 改为公开或获得合适的机器访问方式后，为 `/api/refresh` 配置外部定时器。
3. 如需公开分享，再单独调整 Sites 访问范围；默认保持仅所有者可访问。
4. 用 `analysis_snapshots` 的后续结局字段计算命中率、误报率和平均提前量。
